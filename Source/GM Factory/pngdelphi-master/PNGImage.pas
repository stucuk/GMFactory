...
begin
  ...
  begin
    ...
    {Copy image data}
    CopyMemory(ImageData, TChunkIHDR(Source).ImageData, BytesPerRow * Integer(Height));
        ...
        begin
          ...
          if (AlphaSource[i2] <> 0) then
            if (AlphaSource[i2] = 255) then
              ImageData[i] := TripleToQuad(pRGBTriple(@ImageSource[i2 * 3])^) //STUCUK: Fixed this line. Would crash if last pixel was not transparent due to pRGBQuad typecast (Which was wrong)
            else
              with ImageData[i] do
              begin
