...
procedure MakeComboItemsList(ItemsEx : TComboExItems; var SL : TStringList);
var
 X : Integer;
begin
 SL := TStringList.Create;
 for X := 0 to ItemsEx.Count-1 do
  SL.AddObject(ItemsEx[X].Caption,ItemsEx[X].Data);
end;

procedure SetComboItemsFromList(ItemsEx : TComboExItems; SL : TStringList);
var
 X    : Integer;
 Item : TComboExItem;
begin
 for X := 0 to SL.Count-1 do
  begin
   ...
   Item.Indent             := TShellFolder(Item.Data).Level;
   try
   Item.ImageIndex         := GetShellImage(TShellFolder(Item.Data).AbsoluteID, False, False);//GetShellImageIndex(TShellFolder(Item.Data));
   except
   Item.ImageIndex         := GetShellImage(TShellFolder(Item.Data).AbsoluteID, False, False);//GetShellImageIndex(TShellFolder(Item.Data));
   end;
...
begin
  ...
  try
    ...
    begin
      ...
      SL.InsertObject(Index+NumIDs+1,ItemText,AFolder);
    end;

  SetComboItemsFromList(ItemsEx,SL);
  ...
  try
    ...
    try
      ...
      begin
        ...
        SL.InsertObject(Item+1, AFolder.DisplayName, AFolder);
...
begin
  ...
  try
   //Path := TShellFolder(ItemsEx[ItemIndex].Data).PathName;
//    ItemsEx[ItemIndex].Caption

    //Path := Folders[ItemIndex].PathName;
